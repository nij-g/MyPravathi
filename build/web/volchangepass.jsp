    ...
    <body>
                ...
                if(request.getParameter("volpass") != null){
                    ...
                    if(rs.next()){
                        ...
                        if(existpass.equals(oldpass) && newpass.equals(renewpass)){
                            if(db.changePasswordVolunteer(newpass, session.getAttribute("Volusername").toString())){
                                out.println("<script type='text/javascript'>");
                                out.println("Swal.fire({title: 'Success!',text: 'Succesfully updated!',type: 'success',confirmButtonText: 'Okay', timer: 3000,onAfterClose: function() {location.assign('volunteer.jsp')}})");
                                out.println("</script>");
                            }else{
                                out.println("<script type='text/javascript'>");
                                out.println("Swal.fire({title: 'Error!',text: 'There was some connection error!',type: 'error',confirmButtonText: 'Okay', timer: 3000,onAfterClose: function() {location.assign('volchangepass.jsp')}})");
                                out.println("</script>");
                            }
                        }else{
                            out.println("<script type='text/javascript'>");
                            out.println("Swal.fire({title: 'Error!',text: 'Wrong password or new pass and re-entered password does not match!',type: 'error',confirmButtonText: 'Okay', timer: 3000,onAfterClose: function() {location.assign('volchangepass.jsp')}})");
                            out.println("</script>");
                        }
                    }else{
                            out.println("<script type='text/javascript'>");
        ...
        <div class="container-fluid" style="text-align:center; padding-top:50px; color: #004d4d;">
            ...
            <form action="" method="POST">
                ...
                <input type="text" placeholder="Re-Enter New Password*" autocomplete="off" name="cnewpass" required><br>
                <br><button type="submit" name='volpass' class="mybtn">MAKE CHANGES</button>
            </form>
        </div>
        <% } } %>
    </body>
