<%-- 
    Document   : seekdonation
    Created on : 3 Dec, 2019, 9:25:04 PM
    Author     : hp
--%>
                ...
                <div class="container-fluid">
                    ...
                    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                        <ul class="nav navbar-nav">
                            <li class="active"><a href="#"><span class="glyphicon glyphicon-edit"></span> Seek Donation</a></a></li>
                        </ul>
                    </div>
                </div>
            </nav>
        ...
        <%
            if (request.getParameter("btnSubmit") != null) {
                ...
                if (!isState) {
                    ...
                    out.println("Swal.fire({title: 'Error!',text: 'Please select the correct state!',type: 'error',confirmButtonText: 'Okay', timer: 3000,onAfterClose: function() {location.assign('seekdonation.jsp')}})");
                    out.println("</script>");
                }else if(donationtype.equals("Select your type of Donation")){
                    out.println("<script type='text/javascript'>");
                    out.println("Swal.fire({title: 'Error!',text: 'Please select the donation type!',type: 'error',confirmButtonText: 'Okay', timer: 3000,onAfterClose: function() {location.assign('seekdonation.jsp')}})");
                    out.println("</script>");
                }else if(bankname.equals("Select your bank")){
                    out.println("<script type='text/javascript'>");
                    out.println("Swal.fire({title: 'Error!',text: 'Please select the bank!',type: 'error',confirmButtonText: 'Okay', timer: 3000,onAfterClose: function() {location.assign('seekdonation.jsp')}})");
                    out.println("</script>");
                }else{
                    PravthiDBO dbo = new PravthiDBO();
                    if(dbo.insertDonationSeeker(donationtype, name, email, phone, address, state, amount, bankname, accno, regdate)){
                        out.println("<script type='text/javascript'>");
                        out.println("Swal.fire({title: 'Success!',text: 'Your request has been saved',type: 'success',confirmButtonText: 'OK', timer: 3000,onAfterClose: function() {location.assign('index.jsp')}})");
                        out.println("</script>");  
                    }else {
                        out.println("<script type='text/javascript'>");
                ...
                ResultSet rs1 = db.getBankNames();
        %>
        <div class="loginwhite" style="text-align:center;">
            <h3>Donate Now</h3>
            <h6 style="color:grey">[All the fields are mandatory]</h6>
            <form action="" method="POST">
                <select name="donationtype" class="category" id="issue">
                    <option selected>Select your type of Donation</option>
                    <option value="Educate Children">Educate Children</option>
                    <option value="Donate Food">Donate Food</option>
                    <option value="Proving Shelter">Proving Shelter</option>
                    <option value="Providing Health Care">Providing Health Care</option>
                ...
                <div>
                    <input type="text" placeholder="Enter The Amount required " name="amount" pattern="^(0|[1-9]\d*)$" required>
                </div>
                <div>
                    <select name="bankname" class="category">
                        <option selected>Select your bank</option>
                        <%
                            while(rs1.next()){
                                out.println("<option>"+rs1.getString("bank_name")+"</option>");
                            }
                        %>
                    </select>
                    <input type="text" placeholder="Enter the Account Number" name="accno"  pattern="^\d{9,18}$" required>
                </div>
