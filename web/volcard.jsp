<%-- 
    Document   : volcard
    Created on : 30 Oct, 2019, 12:30:44 AM
    Author     : hp
--%>
        ...
        <%
            ...
            if (flag) {
                ...
                try {
                    ...
                    ResultSet rs;
                    if (name.equals("issuever")) {
                        name = "Issues to be verified";
                        ps = dbo.con.prepareStatement("SELECT * FROM reporttb WHERE verified = 'n'");
                        rs = ps.executeQuery();
                    } else if (name.equals("issuetod")) {
                        String dop;
                        name = "Issues reported today";
                        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd 00:00:00");
                        dop = sdf.format(new Date());
                        ps = dbo.con.prepareStatement("SELECT * FROM reporttb WHERE datetime>=?");
                        ps.setString(1, dop);
                        rs = ps.executeQuery();
                    } else if (name.equals("issueyou")) {
                        name = "Issues verified by you";
                        ps = dbo.con.prepareStatement("SELECT * FROM reporttb WHERE verifiedby=?");
                        ps.setString(1, session.getAttribute("Volusername").toString());
                        rs = ps.executeQuery();
                    } else {
                        name = "Issues marked spam by you";
                        ps = dbo.con.prepareStatement("SELECT * FROM reporttb WHERE verifiedby=? and verified=?");
                        ps.setString(1, session.getAttribute("Volusername").toString());
                        ps.setString(2, "f");
                        rs = ps.executeQuery();
                    }
        ...
        <div>
            <div style="overflow-x:auto;">
                <table class="table table-hover">
                    ...
                    <% do {
                            int id;
                            String issuecat, title, state, phone, mailid, verified, ver, rowcolor, datetime, dateissue, timeissue;
                            id = rs.getInt("id");
